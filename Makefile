#!/bin/bash
.PHONY: astar_pathfinding snake

## General scripts
venv:
	( \
		python3.10 -m pip install --upgrade pip; \
		pip install virtualenv; \
		python3.10 -m venv venv; \
		source venv/bin/activate; \
		pip install -r requirements.txt; \
	)

## CI
test: venv
	( \
		source venv/bin/activate; \
		python3.10 -m unittest; \
	)

synthax_snake: venv
	( \
		source venv/bin/activate; \
		pip install flake8; \
		echo "flake8 analyze :"; \
		flake8 snake --count --select=E9,F63,F7,F82 --show-source --statistics; \
		flake8 snake --count --exit-zero --max-complexity=10 --max-line-length=127 --statistics; \
		echo "flake8 ended."; \
	)

## A* script
astar_pathfinding: venv test
	( \
		source venv/bin/activate; \
		python3.10 main.py astar; \
	)

## Snake game
snake: venv test synthax_snake
	( \
		source venv/bin/activate; \
		python3.10 main.py snake; \
	)

snake_build:
	docker build -f snake/Dockerfile . --tag snake:latest
	docker run snake:latest